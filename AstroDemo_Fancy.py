...
axSKY  = axes([0.15, 0.150, 0.525, 0.02], facecolor=axcolor)
...
def Make_Image(T, S, Dark_strength, Bias_strength, SkyStrength):
    ...
    Sky = np.ones((L,L))*SkyStrength*T
    SKY_noise = np.random.poisson(size = (L,L), lam = Sky**0.5)
    
    # Add all the frames
    D += (Star + Source_noise + Dark + Dark_noise + Bias + Sky + SKY_noise)
    return D
...
# Draw the image
img = Make_Image(T0, S0, Dark_strength0, Bias_strength0, SkyStrength0)
...
def updatefig(val):
    T = sT.val
    S = sS.val
    SkyStrength = sSKY.val
    Dark_strength = sD.val
    NewD = Make_Image(T, S, Dark_strength, Bias_strength0, SkyStrength)
    IMAGE.set_data(NewD)
